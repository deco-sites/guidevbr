...
const statusText: StatusText = {
    ...
    error:
        "Estamos enfrentando problemas técnicos. Clique para mais informações.",
};
...
export default function SystemStatus() {
    ...
    const loadStatus = useCallback(async () => {
        try {
            const response = await fetch(
                "https://status-grm3zrxxba-uc.a.run.app/",
            );
    ...
    const status = services.value.length === 0
        ? "loading"
        : services.value.some(
    ...
    const message = status === "error"
        ...
        } estão com problemas. Clique para mais informações.`
        : status === "success"
    ...
    return (
        <>
            <div
                className="animate-fade-in flex gap-4 items-center mt-1 cursor-pointer"
                onClick={showModal}
            >
                <div className="tooltip" data-tip={message}>
                    <span class="relative flex h-3 w-3">
                        <span
                            class={`${
                                startPing && "animate-ping"
                            } absolute inline-flex h-full w-full rounded-full ${ringColor} opacity-75 transition-colors duration-1000 ease-in-out`}
                        >
                        </span>
                        <span
                            class={`relative inline-flex rounded-full h-3 w-3 ${bgColor} transition-colors duration-1000 ease-in-out`}
                        >
                        </span>
                    </span>
                </div>
            ...
            <dialog
                className="modal bg-stone-500 bg-opacity-25"
                open={openedModal.value}
            >
